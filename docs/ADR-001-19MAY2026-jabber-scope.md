...
Iteration H was added to ROADMAP-001 as a user-requested milestone ("jabber/comp pentesting tooling"). Before sequencing the work, four scope questions needed answers because the framing — *"comp pentesting"* — has more than one defensible interpretation (XMPP only, vs. Cisco UC, vs. SIP/voice, vs. modern team chat). The other three questions were aggressiveness of credential validation, OpenFire CVE-2023-32315 gating, and library policy.
...
**Decision:** Ship a single tool `jabber/openfire-cve-2023-32315.sh` (or `.py`) that:
...
**Decision:** All enumeration and probe scripts in `jabber/` use Python stdlib only (matching the `graphql/gql.py` precedent — single-file portable, no pip install required on the jump box). The OpenFire CVE-2023-32315 helper is HTTP-only and stays stdlib (or `bash + curl`).
...
**Consequence:** `jabber-enum.py`, `jabber-user-enum.py`, `jabber-validate.py`, and `openfire-cve-2023-32315.sh` are stdlib-only / bash-only. No `requirements.txt` in `jabber/`.
