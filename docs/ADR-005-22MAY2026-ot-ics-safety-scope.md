...
### D1 — OT dispatchers live under `ot/`, not `network/`

**Decision:** Tier 4 enumeration scripts go in a new top-level `ot/`
directory. `network/auto-enum.sh` does **not** auto-route to them and
`network/nmap-parse.py` does **not** add OT categories to the auto-routing
`SERVICE_MAP`. Operators must run them by hand or via the new
`ot/ot-enum.sh` orchestrator with explicit `--ics-confirm`.
...
`docs/ROADMAP-003` and run `ot/ot-enum.sh` with `--ics-confirm` only after
...
The orchestrator `ot/ot-enum.sh --ics-confirm` confirms once at the start
...
present in the env (which only `ot/ot-enum.sh` sets after its prompt).
...
single PLC, ever). The orchestrator `ot/ot-enum.sh` parallelizes at the host
...
**ignored** in `ot/`; the throttle floor is non-negotiable.
...
**Consequence:** an `ot/ot-enum.sh` run against a /24 will take meaningfully
longer than the equivalent `network/auto-enum.sh`. This is intentional.
...
**Decision:** `network/report.py` (or a new `ot/report.py` — see ROADMAP-003)
...
| Document | Status |
|---|---|
| `docs/ROADMAP-003-22MAY2026-tier4-ics-enumeration.md` | Drafted alongside this ADR |
| `ot/` directory creation | Tracked in ROADMAP-003 §implementation |
| Update to `docs/ROADMAP-001-19MAY2026-thoroughness-execution.md` iteration I-A | Mark "deferred to T4 / ADR-005" with link |
| Update to `docs/ROADMAP-002-22MAY2026-tier1-tier2-enumeration.md` Tier 4 section | Cross-link to ADR-005 + ROADMAP-003 |
