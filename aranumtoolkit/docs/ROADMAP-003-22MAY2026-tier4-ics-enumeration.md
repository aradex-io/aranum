...
`standalones/ot/` directory, no auto-routing.
...
|---|---|
| **Status** | ⬜ |
| **Files** | `standalones/ot/` (new top-level dir), `standalones/ot/ot-enum.sh`, `standalones/ot/_lib.sh`, `standalones/ot/README.md` |
| **Plan** | (1) Create `standalones/ot/` with a README that opens with the ADR-005 safety section verbatim. (2) `standalones/ot/_lib.sh` defines `OT_THROTTLE_MIN_MS=500` (floor, non-overridable), `OT_MAX_PARALLEL=2` (default), `OT_MAX_PARALLEL_HARD=4` (ceiling), `ot_confirm()` helper that prompts for the literal string `ICS-CONFIRMED`, `ot_throttle_sleep()` that respects the floor. (3) `standalones/ot/ot-enum.sh --ics-confirm` is the only auto-routing entrypoint; takes a `--targets` file of `ip:port/proto` triples (no nmap-XML auto-route — operator must hand-pick). |
| **Acceptance** | `standalones/ot/ot-enum.sh` without `--ics-confirm` exits with the safety message and rc=2. With `--ics-confirm`, prompts for the typed confirmation. Anything other than the literal `ICS-CONFIRMED` typed exactly exits rc=2. Confirmation sets `OT_CONFIRMED=1` in the dispatcher subprocess env. |
| **Commit** | `feat(ot): add standalones/ot/ orchestrator skeleton with double-gated safety prompt` |
...
|---|---|
| **Status** | ⬜ |
| **Files** | `standalones/ot/enum-modbus.sh` |
...
|---|---|
| **Status** | ⬜ |
| **Files** | `standalones/ot/enum-s7.sh` |
...
|---|---|
| **Status** | ⬜ |
| **Files** | `standalones/ot/enum-enip.sh` |
...
|---|---|
| **Status** | ⬜ |
| **Files** | `standalones/ot/enum-bacnet.sh` |
...
|---|---|
| **Status** | ⬜ |
| **Files** | `standalones/ot/enum-opcua.sh` |
...
|---|---|
| **Status** | ⬜ |
| **Files** | `standalones/ot/enum-dnp3.sh` |
...
|---|---|
| **Status** | ⬜ |
| **Files** | `standalones/ot/enum-iec104.sh` |
...
|---|---|
| **Status** | ⬜ |
| **Files** | `aranumtoolkit/network/report.py` (extend with OT severity rules) |
...
|---|---|
| **Status** | ⬜ |
| **Files** | `aranumtoolkit/network/nmap-parse.py` |
| **Plan** | Add an `ot-untouched` category that matches OT ports (502, 102, 44818, 47808, 4840, 20000, 2404) and emits a one-line hint pointing to `aranumtoolkit/docs/ROADMAP-003` and `standalones/ot/ot-enum.sh --ics-confirm`. **Crucially:** the category is **not** wired into `SERVICE_MAP`'s normal dispatch path — `auto-enum.sh` sees the hint and prints it; it does NOT invoke any `standalones/ot/enum-*.sh`. |
...
|---|---|
| **Status** | ⬜ |
| **Files** | `deps-check.sh`, `aranumtoolkit/tests/fp-harness.sh`, `aranumtoolkit/tests/tp-server.py` |
| **Plan** | (1) `deps-check.sh` — add OT section asserting `nmap` (NSE scripts present), `python3 stdlib socket` (for T4.7), and the `OT_CONFIRMED` env var documentation. (2) `aranumtoolkit/tests/tp-server.py` — add an `--ot-enip` stub that replies to a single List Identity (0x0063) packet, used by T4.3 acceptance. (3) `aranumtoolkit/tests/fp-harness.sh` — extend the ENV-GATE TEST block to also assert each OT dispatcher refuses without `OT_CONFIRMED=1`. Cross-service FP test (from Phase 3 below) covers OT-vs-HTTP. |
...
|---|---|
| **Status** | ⬜ |
| **Files** | `standalones/ot/README.md`, top-level `README.md`, `CHANGELOG.md` |
| **Plan** | (1) `standalones/ot/README.md` — opens with ADR-005 safety section verbatim, then the dispatcher index, then a "how-to" that ends with "if any of the above is unclear, do not run these tools." (2) Top-level `README.md` — add an `standalones/ot/` row to the layout tree, with a warning callout that auto-routing is disabled. (3) `CHANGELOG.md` — roll `[Unreleased]` → `[v0.23.0]` with the T4 deliverables. |
