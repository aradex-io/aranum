...
**Source of truth:** `aranumtoolkit/docs/REVIEW-001-19MAY2026-thoroughness-audit.md`
...
| **I-C / I-F / I-G / I-H** | **FlexNet license servers + HPC schedulers + monitoring + backup detection** — four `aranumtoolkit/network/` dispatchers in one MINOR bump | ~1 day | ✅ done 2026-05-23 | `v0.25.0` |
...
| **Files** | `aranumtoolkit/network/_lib.sh`, `aranumtoolkit/network/enum-smb.sh`, `aranumtoolkit/network/enum-ldap.sh`, `aranumtoolkit/network/enum-mssql.sh`, `aranumtoolkit/network/enum-winrm.sh`, `aranumtoolkit/network/enum-rdp.sh` |
...
| **Acceptance** | (1) `grep -rn 'NXC_ARGS+=" -u'` returns no hits. (2) `grep -rn 'creds="-u' aranumtoolkit/network/` returns no hits. (3) Each dispatcher passes a smoke test with a credential containing a single quote: `ENUM_USER="al'ice"`. (4) `bash -n` parses every modified file. |
...
| **Files** | `aranumtoolkit/network/nmap-parse.py`, `deps-check.sh` |
...
| **Acceptance** | (1) Existing `aranumtoolkit/network/test.xml` still parses to identical output (byte-compare `nmap-parse.py test.xml --json` before/after). (2) A crafted `aranumtoolkit/tests/fixtures/malicious.xml` with `<!DOCTYPE foo [<!ENTITY x SYSTEM "file:///etc/passwd">]>` is rejected with a clear error and zero file reads. (3) Works on a host without defusedxml installed (verified by `pip uninstall -y defusedxml` and re-run). |
...
| **Files** | `standalones/graphql/gql.py` |
...
| **Files** | `aranumtoolkit/network/enum-ldap.sh`, audit pass on `enum-ssh.sh`, `enum-mssql.sh`, `enum-redis.sh`, `enum-winrm.sh`, `enum-rdp.sh` |
...
Smoke: rerun `bash deps-check.sh`, `python3 -m py_compile aranumtoolkit/network/nmap-parse.py standalones/graphql/gql.py`, and execute each modified dispatcher with `--targets /tmp/empty.txt --output /tmp/out` (smoke-grade: confirms argparse + arg-handling don't regress).
...
|---|---|---|---|---|
| B.1 | PostgreSQL | 5432 | `aranumtoolkit/network/enum-postgres.sh` | ✅ done 2026-05-19 |
| B.2 | MySQL/MariaDB | 3306 | `aranumtoolkit/network/enum-mysql.sh` | ✅ done 2026-05-19 |
| B.3 | MongoDB | 27017–27019 | `aranumtoolkit/network/enum-mongo.sh` | ✅ done 2026-05-19 |
| B.4 | Elasticsearch/Kibana | 9200, 5601 | `aranumtoolkit/network/enum-elastic.sh` | ✅ done 2026-05-19 |
| B.5 | Docker API | 2375, 2376 | `aranumtoolkit/network/enum-docker.sh` | ✅ done 2026-05-19 |
| B.6 | Kubernetes API/kubelet | 6443, 8080, 10250 | `aranumtoolkit/network/enum-kubernetes.sh` | ✅ done 2026-05-19 |
| B.7 | IPMI/BMC | 623/udp | `aranumtoolkit/network/enum-ipmi.sh` | ✅ done 2026-05-19 |
| B.8 | nmap-parse categories (docker + kubernetes; others already present) | — | `aranumtoolkit/network/nmap-parse.py` | ✅ done 2026-05-19 |
| B.9 | Release + docs | — | README + CHANGELOG + tag | ✅ done 2026-05-19 |

**Each dispatcher must:**
- Source `aranumtoolkit/network/_lib.sh` and use `parse_common_args`.
...
|---|---|---|---|
| C.1 | VNC dispatcher | `aranumtoolkit/network/enum-vnc.sh` | ✅ done 2026-05-19 |
| C.2 | JMX/RMI dispatcher | `aranumtoolkit/network/enum-jmx.sh` | ✅ done 2026-05-19 |
| C.3 | RabbitMQ mgmt | `aranumtoolkit/network/enum-rabbitmq.sh` | ✅ done 2026-05-19 |
| C.4 | Memcached | `aranumtoolkit/network/enum-memcached.sh` | ✅ done 2026-05-19 |
| C.5 | CouchDB | `aranumtoolkit/network/enum-couchdb.sh` | ✅ done 2026-05-19 |
| C.6 | etcd | `aranumtoolkit/network/enum-etcd.sh` | ✅ done 2026-05-19 |
| C.7 | HTTP: JWT extract + alg-confusion probe | `aranumtoolkit/network/enum-http.sh` | ✅ done 2026-05-19 |
| C.8 | HTTP: CORS misconfig probe | `aranumtoolkit/network/enum-http.sh` | ✅ done 2026-05-19 |
| C.9 | HTTP: exposed VCS dir check | `aranumtoolkit/network/enum-http.sh` | ✅ done 2026-05-19 |
| C.10 | HTTP: virtual-host fuzz mode | `aranumtoolkit/network/enum-http.sh` | ✅ done 2026-05-19 |
| C.11 | HTTP: cert collection + SANs | `aranumtoolkit/network/enum-http.sh` | ✅ done 2026-05-19 |
| C.12 | HTTP: micro-wordlist of high-value paths | `aranumtoolkit/network/enum-http.sh` | ✅ done 2026-05-19 |
| C.13 | SMB: NTLM-relay viability + PetitPotam coerce signal | `aranumtoolkit/network/enum-smb.sh` | ✅ done 2026-05-19 |
| C.14 | SSH: CVE-2018-15473 user enum + key-only refusal | `aranumtoolkit/network/enum-ssh.sh` | ✅ done 2026-05-19 |
...
| Item | Scope | Files | Status |
|---|---|---|---|
| D.1 | `Get-LAPSPassword.ps1` | `standalones/windows/` | ✅ shipped in D1 / `v0.17.0` |
| D.2 | `Get-ADCSMisconfig.ps1` (ESC1/2/4 via ADSI) | `standalones/windows/` | ✅ covered by D1 LDAP-side Certipy ingest (D.9); pure-PowerShell ADSI variant ❌ declined (Certipy is the maintained path) |
| D.3 | `Get-GPPCPassword.ps1` (SYSVOL `cpassword=`) | `standalones/windows/` | ✅ shipped in D1 / `v0.17.0` |
| D.4 | `Get-DPAPIBlobs.ps1` (locations only — no decrypt) | `standalones/windows/` | ✅ shipped in D1 / `v0.17.0` |
| D.5 | `Get-NamedPipes.ps1` (ACL enumeration) | `standalones/windows/` | ✅ shipped in D1 / `v0.17.0` |
| D.6 | `Get-PrintNightmare.ps1` (registry + Spooler state) | `standalones/windows/` | ✅ shipped in D1 / `v0.17.0` |
| D.7 | `Test-CoercedAuth.ps1` (local-only safety probe) | `standalones/windows/` | ✅ shipped in D1 / `v0.17.0` |
| D.8 | LDAP: BloodHound ingestion when creds + DC IP present | `aranumtoolkit/network/enum-ldap.sh` | ✅ shipped in D1 / `v0.17.0` (ADR-004 D1) |
| D.9 | LDAP: Certipy `find` for ADCS templates | `aranumtoolkit/network/enum-ldap.sh` | ✅ shipped in D1 / `v0.17.0` (ADR-004 D2) |
| D.10 | LDAP: delegation enum (constrained/unconstrained/RBCD) | `aranumtoolkit/network/enum-ldap.sh` | ✅ shipped in D1 / `v0.17.0` |
| D.11 | Linux: `pwnkit-check.sh`, `looney-check.sh`, `overlayfs-check.sh`, `io-uring-check.sh`, `namespaces-check.sh` | `standalones/linux/` | ✅ shipped in D2 / `v0.18.0` |
...
| Item | Scope | Files | Status |
|---|---|---|---|
| E.1 | `report.py` — unified Markdown / JSON / HTML report | `aranumtoolkit/network/report.py` (new) | ✅ done 2026-05-19 |
| E.2 | `autoenum-diff.sh` — diff two run outputs | `aranumtoolkit/network/autoenum-diff.sh` (new) | ✅ done 2026-05-19 |
| E.3 | Central `run.log` with tool versions + invocations + exit codes | `aranumtoolkit/network/auto-enum.sh` + dispatchers | ✅ done 2026-05-19 |
| E.4 | `--resume` flag — per-service `.done` markers | `aranumtoolkit/network/auto-enum.sh` | ✅ done 2026-05-19 |
| E.5 | SOCKS / HTTP proxy support across the suite | `aranumtoolkit/network/_lib.sh`, `standalones/graphql/gql.py` | ✅ done 2026-05-19 (gql.py in F.7; aranumtoolkit/network/_lib.sh helpers added) |
| E.6 | User-Agent customization + `--ua-rotate` | `standalones/graphql/gql.py`, curl callsites | ✅ done 2026-05-19 (gql.py in F.8; `curl_ua()` helper for dispatchers) |
...
| E.10 | `auto-enum.sh` dispatcher-failure tally at end | `aranumtoolkit/network/auto-enum.sh` | ✅ done 2026-05-19 |
...
| Item | Scope | Files | Status |
|---|---|---|---|
| F.1 | Field-suggestion harvest when introspection disabled | `standalones/graphql/gql.py` (new subcommand `suggest`) | ✅ done 2026-05-19 |
| F.2 | Batched-query support (`[{query:...}, ...]`) | `standalones/graphql/gql.py` (`--batch` on `call`/`raw`) | ✅ done 2026-05-19 |
| F.3 | Persisted-query bypass probe (APQ) | `standalones/graphql/gql.py` (new subcommand `apq-probe`) | ✅ done 2026-05-19 |
| F.4 | `@skip` / `@include` directive abuse helper | `standalones/graphql/gql.py` (`--directive-bypass` on `diff`) | ✅ done 2026-05-19 |
| F.5 | CSRF-via-GET probe | `standalones/graphql/gql.py` (new subcommand `csrf-probe`) | ✅ done 2026-05-19 |
| F.6 | Alias-DoS detection (latency growth measurement; detect-only) | `standalones/graphql/gql.py` (`--alias-dos-check` on `call`) | ✅ done 2026-05-19 |
| F.7 | SOCKS proxy support | `standalones/graphql/gql.py --proxy` + env | ✅ done 2026-05-19 |
| F.8 | User-Agent flag + rotation | `standalones/graphql/gql.py --user-agent / --ua-rotate / GQL_UA` | ✅ done 2026-05-19 |
...
| Item | Scope | Files | Status |
|---|---|---|---|
| G.1 | `aranumtoolkit/tests/fixtures/services/` synthetic nmap-XML banners per service | new | ✅ done 2026-05-20 |
| G.2 | `aranumtoolkit/tests/test_nmap_parse.py` unit tests (19 tests) | new | ✅ done 2026-05-20 |
| G.3 | `aranumtoolkit/tests/test_gql_internals.py` — pure-function tests for gql.py (28 tests; renamed from `test_gql_query_builder.py` because the testable seams are `cache_key`, `_check_range_size`, `type_str`, `unwrap`, `type_by_name`, `parse_kv_args`, `build_selection` — not an abstraction we'd invent) | new | ✅ done 2026-05-20 |
...
| G.7 | `--throttle` mode for sensitive environments; operator-explicit CLI wins over throttle defaults; `throttle_delay()` / `throttle_sleep()` / `throttle_nmap_args()` helpers in `_lib.sh` | `aranumtoolkit/network/auto-enum.sh`, `aranumtoolkit/network/_lib.sh` | ✅ done 2026-05-20 |
| G.8 | Engagement-scoped-write audit (CLAUDE.md §9 invariant 1); 6 helpers gained explicit `--exploit` / `--write` / `--send` gates; findings + methodology in [REVIEW-002](REVIEW-002-20MAY2026-write-gate-audit.md) | `standalones/activemq/`, `standalones/redis/`, `standalones/smtp/`, `aranumtoolkit/docs/` | ✅ done 2026-05-20 |
...
- **D4** stdlib-only Python (matches `gql.py` precedent). No `requirements.txt` in `standalones/jabber/`.
...
| Item | Scope | Files | Status |
|---|---|---|---|
| H.1 | Add XMPP/Jabber service categories to nmap-parse: 5222 (c2s STARTTLS), 5223 (legacy implicit-TLS c2s), 5269 (s2s), 5280/5281 (BOSH / WebSocket / HTTP-bind), 5298 (link-local), 7777 (Openfire file-transfer proxy), 9090/9091 (Openfire admin console). Two categories: `xmpp` and `openfire-admin` so the dispatchers can route differently. | `aranumtoolkit/network/nmap-parse.py` | ✅ done 2026-05-19 |
| H.2 | `enum-jabber.sh` dispatcher routed automatically by `auto-enum.sh` based on H.1 categorization. Phases: (a) raw banner via `nc`, (b) STARTTLS feature probe + cert collection (SANs feed domain inventory), (c) advertised SASL mechanisms parse, (d) XEP-0077 in-band-registration probe (no actual register — just `<iq type='get'><query xmlns='jabber:iq:register'/></iq>` to confirm IBR is advertised), (e) server `disco#info` + `disco#items`, (f) MUC discovery on `conference.<domain>` with anonymous-and-then-auth attempts, (g) BOSH/WebSocket endpoint detection on 5280/5281. All output to `$OUT/<ip>/`. | `aranumtoolkit/network/enum-jabber.sh` (new), sources `aranumtoolkit/network/_lib.sh` | ✅ done 2026-05-19 |
| H.3 | `jabber-user-enum.py` — SASL response differential as the primary read-only enum technique. Per-user verdict (USER_EXISTS / EXISTS_DISABLED / EXISTS_EXPIRED / INVALID_FORMAT / NO_PLAIN / SERVER_ERROR / UNCLASSIFIED / STREAM_FAIL) + per-user response time. Timing fallback documented for hardened servers that return identical `<not-authorized/>` for every name. Stdlib-only (per ADR-001 D4). XEP-0077 IBR-based probing **deliberately not used** — it creates accounts when the conflict-error path doesn't fire (write side-effect; ADR-001 D2 violation). MUC nick harvest + vCard lookup deferred to a follow-up; this version focuses on the strongest signal (SASL differential). Output: per-user JSONL via `--out` + final summary table. | `standalones/jabber/jabber-user-enum.py` (new) | ✅ done 2026-05-19 |
| H.4 | `jabber-validate.py` — single-credential validation only (per ADR-001 D2 — **no spray**). Accepts `--jid` + `--password`, attempts SASL PLAIN / SCRAM-SHA-1 / SCRAM-SHA-256 against the host, confirms auth or surfaces the SASL failure reason (`<not-authorized/>`, `<account-disabled/>`, etc.). Stdlib-only. SCRAM verified against RFC 5802 §5 reference vector. | `standalones/jabber/jabber-validate.py` (new) | ✅ done 2026-05-19 |
| H.5 | `openfire-cve-2023-32315.py` — default behavior: detection only (probes the path-traversal signature, reports version + verdict). `--exploit`: typed-FQDN confirm → admin create via the bypass → JSP webshell plugin upload → returns webshell URL + writes `<out>/exploit_log.txt` with admin-username, plugin-name, timestamp. `--cleanup <log>`: takes the log from a prior `--exploit` run, deletes the admin user via the legitimate REST API, removes the plugin. README must document `--cleanup` before `--exploit`. Stdlib (urllib) + bash glue. Per ADR-001 D3. | `standalones/jabber/openfire-cve-2023-32315.py` (new) | ✅ done 2026-05-19 (detect mode complete; exploit + cleanup paths written per public PoC pattern and explicitly marked REQUIRES_LAB_VERIFICATION against a known-vulnerable 4.7.4 container before engagement use — see README §lab-verify) |
| H.6 | Ejabberd / Prosody admin-API exposure check — detection only. Probes Ejabberd commands API on `<host>/api/` (returns 401 vs 200 vs 404 — flag if any method responds without auth), Prosody `mod_admin_telnet` on TCP 5582, `mod_admin_web` on 5280/admin. | `standalones/jabber/jabber-admin-api-probe.sh` (new), wired into `enum-jabber.sh` as a final phase | ✅ done 2026-05-19 |
| H.7 | README + workflow doc. Order of sections: safety invariants (from ADR-001), `--cleanup` before `--exploit` for the OpenFire helper, common workflows (enum → user-enum → validate → optionally OpenFire), tool index. | `standalones/jabber/README.md` (new) | ✅ done 2026-05-19 |
...
| H.9 | Top-level `README.md` — add `standalones/jabber/` row to the layout tree + scripts table, and `enum-jabber.sh` row under network dispatchers. | `README.md` | ✅ done 2026-05-19 |
...
- H.9: top-level README's layout tree includes `standalones/jabber/`; scripts table includes the four jabber helpers and the dispatcher.
...
- Release: `chore(repo): release v0.9.0 — iteration H standalones/jabber/XMPP tooling`
...
- Default-cred brute-force against BMC at scale — single-cred validation only, same policy as Jabber (ADR-001 D2). `standalones/creds/default-creds-sweep.py` already exists for vendor-default sweeps and is the right place for that work.
- Anything that modifies HPC scheduler state (submit jobs, cancel jobs).

**Pre-execution requirement (original):** `aranumtoolkit/docs/ADR-002-<DATE>-internal-pentest-scope.md` resolving questions 1–5 and committing to a top-12-15 protocol list for v0.16.0. Operator approves before code lands. *(Not written — the ADR-002 slot was consumed by [ADR-002 bulk-enum](ADR-002-20MAY2026-bulk-enum-design.md). The reconciliation table below replaces it.)*
...
|---|---|---|
| **I-A** Industrial / OT (Modbus, S7, EtherNet/IP, BACnet, OPC-UA, DNP3, IEC 60870-5-104) | ✅ shipped as **T4 / `v0.24.0`** via [ADR-005](ADR-005-22MAY2026-ot-ics-safety-scope.md) + [ROADMAP-003](ROADMAP-003-22MAY2026-tier4-ics-enumeration.md) | Read-side identification only; `standalones/ot/ot-enum.sh` orchestrator with `--ics-confirm` typed gate; 500ms throttle floor; 7 protocol dispatchers |
...
| **I-C** License servers (FlexNet, HASP, RLM, LUM, LabVIEW VI Server) | ✅ FlexNet shipped in `v0.25.0` via `aranumtoolkit/network/enum-flexnet.sh` (HASP / RLM / LUM / LabVIEW VI deferred) | Banner + `lmutil lmstat -a` when available |
...
| **I-F** HPC scheduler / cluster mgmt (Slurm, PBS, SGE, HTCondor, Spark, HDFS, YARN) | ✅ Slurm + HTCondor + YARN shipped in `v0.25.0` via `aranumtoolkit/network/enum-hpc.sh` (PBS / SGE deferred); Spark UI + Hadoop NameNode shipped earlier in E3 (`v0.21.0`) | Read-side only; no job submission |
| **I-G** Scientific / lab data (DICOM, HL7, Splunk, Grafana, InfluxDB, Zabbix, Nagios NRPE) | ✅ Zabbix + NRPE + Splunk shipped in `v0.25.0` via `aranumtoolkit/network/enum-monitoring.sh`; Grafana + Prometheus + InfluxDB shipped earlier | DICOM 104/11112 + HL7 MLLP 2575 deferred (specialist, low engagement-frequency) |
| **I-H** Backup infrastructure (Veeam, CommVault, NetBackup) | ✅ shipped in `v0.25.0` via `aranumtoolkit/network/enum-backup.sh` | Detection-only at this tier; HIGH severity because reaching backup tier from user VLAN is itself a segmentation finding |
