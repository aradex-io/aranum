...
1. Drop file in `aranumtoolkit/network/enum-<svc>.sh` following the `enum-memcached.sh` template (set -uo pipefail, source _lib.sh, parse_common_args, throttle_sleep where appropriate, emit `_hints.txt`).
2. Add severity rule(s) to `aranumtoolkit/network/report.py` so emitted `UNAUTH:` and version-banner lines are graded.
...
### SERVICE_MAP additions (`aranumtoolkit/network/nmap-parse.py`)
...
> Tests: `aranumtoolkit/tests/test_nmap_parse.py` must add fixtures for each new category. ref obs 8802 (last time, a SERVICE_MAP add broke routing tests because dispatch() entries needed manual decoration).
