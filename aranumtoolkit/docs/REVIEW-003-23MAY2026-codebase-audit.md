...
1. **Python correctness** — `standalones/graphql/gql.py`, `aranumtoolkit/network/nmap-parse.py`,
   `aranumtoolkit/network/report.py`, `aranumtoolkit/network/report-dashboard.py`,
   `aranumtoolkit/network/bulk-enum-windows.py`, `standalones/creds/*.py`, `standalones/jabber/*.py`,
   `standalones/activemq/activemq-cve-2023-46604.py`, `standalones/redis/redis-rogue-master.py`,
   `standalones/smtp/smtp-smuggling-test.py`.
2. **Shell correctness** — entire `aranumtoolkit/network/enum-*.sh` fleet (~60),
   `aranumtoolkit/network/_lib.sh`, `aranumtoolkit/network/auto-enum.sh`, `aranumtoolkit/network/bulk-enum-linux.sh`,
   `standalones/ot/_lib.sh`, `standalones/ot/ot-enum.sh`, all `_*_lib.sh` shared libs.
3. **Safety / OPSEC** — every state-modifying tool (`*-rce-*`, `*-cve-*`,
   `smtp-phish-send`, `openfire-cve-*`, OT/ICS gate, E4 aggressive UDP).
4. **PowerShell + tests + docs sync** — `standalones/windows/*.ps1`, `aranumtoolkit/tests/test_*.py`,
   `aranumtoolkit/tests/smoke.sh`, README ↔ filesystem ↔ CHANGELOG cross-check.
...
| C2 | Critical | `aranumtoolkit/tests/smoke.sh` hardcoded `/home/jay/...` → `make smoke` red on any other host | Derived REPO from `$BASH_SOURCE` |
| C3 | High (security) | `aranumtoolkit/network/enum-smb.sh` wrote to fixed `/tmp/relay_cand.tmp` (symlink/race risk) | `mktemp` + trap cleanup |
...
- **`aranumtoolkit/network/nmap-parse.py` "`r'a^'` regex bug"** — the `a^` regex is
...
- **104 `SC2046` shellcheck warnings** across the dispatcher fleet, all
  ...
  `$(throttle_nmap_args)` word-split pattern in `aranumtoolkit/network/_lib.sh`. The
