...
(`aranumtoolkit/network/bulk-enum-linux.sh`) over SSH stdin-pipe at v0.15.0. ADR-002 D6
...
on a 50-500 host internal network, run `standalones/windows/Invoke-PrivEscEnum.ps1`
...
**Decision:** `aranumtoolkit/network/bulk-enum-windows.py` connects via WinRM (port 5985 HTTP
...
**Decision:** `aranumtoolkit/network/bulk-enum-windows.py` is Python. `pywinrm` is the
canonical WinRM client; bashing it (calling `python -c` from bash) adds
no value.

**Reason:** consistent with `standalones/graphql/gql.py`, `standalones/smtp/smtp-smuggling-test.py`,
`standalones/creds/default-creds-sweep.py`, `standalones/jabber/jabber-validate.py`,
...
the per-CVE helpers (`standalones/jabber/openfire-cve-2023-32315.py --exploit`,
`standalones/activemq/activemq-cve-2023-46604.py --exploit`, etc.) which carry their
own gates per G.8.
