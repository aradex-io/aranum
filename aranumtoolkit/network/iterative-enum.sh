...
# output tree. All filesystem operations are read-only.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
PARSER="$SCRIPT_DIR/nmap-parse.py"
DEFAULT_CREDS="$PROJECT_ROOT/standalones/creds/default-creds-sweep.py"
JUICY_SCRIPT="$PROJECT_ROOT/standalones/linux/juicy-files-hunt.sh"
...
Tuning:
  ...
  --no-default-creds      skip standalones/creds/default-creds-sweep.py
...
phase_http_source() {
    ...
    cp "$HTTP_URLS" "$OUTDIR/burp-scope.txt"
    : > "$OUTDIR/http/source_products.tsv"
...
- Authenticated web inventory: with supplied standalones/creds/cookies, crawl admin portals for users, tokens, integrations, webhooks, runners, package repos, and backup configs.
