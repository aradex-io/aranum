#!/usr/bin/env python3
"""test_nmap_parse.py — G.2 unit tests for aranumtoolkit/network/nmap-parse.py.

Run with: python3 -m unittest tests.test_nmap_parse
or:       python3 aranumtoolkit/tests/test_nmap_parse.py
...
Fixtures live in aranumtoolkit/tests/fixtures/services/ and aranumtoolkit/tests/fixtures/*.xml
...
PARSER_PATH = REPO / "aranumtoolkit" / "network" / "nmap-parse.py"
FIXTURES = REPO / "aranumtoolkit" / "tests" / "fixtures" / "services"
MALICIOUS_DIR = REPO / "aranumtoolkit" / "tests" / "fixtures"
...
class TestParserParity(unittest.TestCase):
    ...
    that come from a real-ish scan (committed in aranumtoolkit/network/)."""

    def test_xml_vs_gnmap_host_count(self):
        if not (REPO / "aranumtoolkit" / "network" / "test.gnmap").exists():
            self.skipTest("aranumtoolkit/network/test.gnmap missing")
        xml_hosts  = {e["ip"] for e in M.dispatch(REPO / "aranumtoolkit" / "network" / "test.xml")}
        gnmap_hosts = {e["ip"] for e in M.dispatch(REPO / "aranumtoolkit" / "network" / "test.gnmap")}
