...
for py in standalones/graphql/gql.py standalones/creds/default-creds-sweep.py aranumtoolkit/network/nmap-parse.py \
          standalones/jabber/jabber-user-enum.py standalones/jabber/jabber-validate.py \
          standalones/jabber/openfire-cve-2023-32315.py standalones/activemq/activemq-cve-2023-46604.py \
          standalones/smtp/smtp-smuggling-test.py standalones/redis/redis-rogue-master.py ; do
    [ -x "$py" ] && help_works "python3 $py" || s "skipped: $py not executable"
done
# Bash tools with --help (some respond to -h)
for sh in aranumtoolkit/network/auto-enum.sh standalones/jabber/jabber-admin-api-probe.sh; do
...
# -----------------------------------------------------------------
if grep -rqn 'NXC_ARGS+=" -u' aranumtoolkit/network/ 2>/dev/null; then
    f "shell-injection: NXC_ARGS+= pattern detected"
else
    p "shell-injection: no NXC_ARGS+= patterns (A.1 holds)"
fi
if grep -rqn 'creds="-u' aranumtoolkit/network/ 2>/dev/null; then
...
out=$(bash -c '. aranumtoolkit/network/_lib.sh; export ENUM_USER="al'\''ice"; export ENUM_PASS="p\$x"; args=(); nxc_creds_array args; printf "%s\n" "${args[@]}"' 2>&1)
if [ "$(printf "%s\n" "$out" | wc -l)" -eq 4 ] \
   && printf "%s\n" "$out" | grep -qx "al'ice" \
...
# ldap_url helper
v4=$(bash -c '. aranumtoolkit/network/_lib.sh; ldap_url 10.0.0.1')
v6=$(bash -c '. aranumtoolkit/network/_lib.sh; ldap_url "2001:db8::1" 636 ldaps')
...
# -----------------------------------------------------------------
for fix in aranumtoolkit/network/test.xml aranumtoolkit/network/test.gnmap aranumtoolkit/network/test.nmap; do
    if python3 aranumtoolkit/network/nmap-parse.py "$fix" --json >/dev/null 2>&1; then
        p "parse benign: $fix"
    else
...
benign_json=$(python3 aranumtoolkit/network/nmap-parse.py aranumtoolkit/network/test.xml --json)
...
# -----------------------------------------------------------------
for fix in aranumtoolkit/tests/fixtures/malicious_xxe.xml aranumtoolkit/tests/fixtures/billion_laughs.xml; do
    if timeout 10 python3 aranumtoolkit/network/nmap-parse.py "$fix" --json >/dev/null 2>&1; then
...
spec = importlib.util.spec_from_file_location('m', 'aranumtoolkit/network/nmap-parse.py')
...
EOF
xmpp_t=$(python3 aranumtoolkit/network/nmap-parse.py /tmp/test_xmpp_route.xml --service xmpp)
of_t=$(python3 aranumtoolkit/network/nmap-parse.py /tmp/test_xmpp_route.xml --service openfire-admin)
...
# -----------------------------------------------------------------
if python3 standalones/graphql/gql.py --help 2>&1 | grep -q "insecure"; then
...
python3 -c "
import importlib.util
spec = importlib.util.spec_from_file_location('gql', 'standalones/graphql/gql.py')
m = importlib.util.module_from_spec(spec); spec.loader.exec_module(m)
...
out=$(python3 standalones/graphql/gql.py --url http://127.0.0.1:1/graphql loop currentUser --vary id --range 1-2000000 --no-schema 2>&1)
...
out=$(timeout 10 python3 standalones/jabber/jabber-user-enum.py --host 127.0.0.1 --port 1 --domain x --user-list /tmp/u.txt 2>&1)
...
out=$(timeout 10 python3 standalones/jabber/jabber-validate.py --host 127.0.0.1 --port 1 --domain x --jid alice --password test 2>&1)
rc=$?
...
out=$(timeout 10 python3 standalones/jabber/openfire-cve-2023-32315.py --url http://127.0.0.1:1 detect 2>&1)
rc=$?
...
echo "" > /tmp/e.txt
bash aranumtoolkit/network/enum-jabber.sh --targets /tmp/e.txt --output /tmp/jt-test >/dev/null 2>&1
...
for svc in postgres mysql mongo elastic docker kubernetes ipmi \
    ...
    rm -rf "$out_dir"
    if timeout 15 bash aranumtoolkit/network/enum-$svc.sh --targets /tmp/empty.txt --output "$out_dir" >/dev/null 2>&1; then
...
for svc in $GATED; do
    ...
    if timeout 10 bash aranumtoolkit/network/enum-$svc.sh --targets /tmp/empty.txt --output "$out_dir" >/dev/null 2>&1; then
...
EOF
docker_route=$(python3 aranumtoolkit/network/nmap-parse.py /tmp/test_route_b.xml --service docker 2>/dev/null)
k8s_route=$(python3 aranumtoolkit/network/nmap-parse.py /tmp/test_route_b.xml --service kubernetes 2>/dev/null)
...
out=$(timeout 8 python3 standalones/graphql/gql.py --url http://127.0.0.1:1/graphql suggest --corpus /dev/null 2>&1)
...
out=$(timeout 3 python3 standalones/graphql/gql.py --url http://127.0.0.1:1/graphql --proxy http://127.0.0.1:65535 csrf-probe 2>&1)
...
# report.py
if python3 aranumtoolkit/network/report.py "$fake" --label smoke >/dev/null 2>&1; then
...
bash aranumtoolkit/network/autoenum-diff.sh "$A" "$B" >/dev/null 2>&1
...
EOF
if python3 aranumtoolkit/network/report-dashboard.py --output "$dash" "$fake" >/dev/null 2>&1; then
...
grep -q -- "--resume" aranumtoolkit/network/auto-enum.sh && p "auto-enum.sh: --resume present" \
                                            || f "auto-enum.sh: --resume missing"
grep -q "run_log" aranumtoolkit/network/auto-enum.sh && p "auto-enum.sh: run_log writer present" \
                                       || f "auto-enum.sh: run_log writer missing"
grep -q "Dispatcher results:" aranumtoolkit/network/auto-enum.sh && p "auto-enum.sh: failure tally present" \
                                                   || f "auto-enum.sh: failure tally missing"
tmp_plan=/tmp/aratool-plan-smoke.$$
rm -rf "$tmp_plan"
if bash aranumtoolkit/network/auto-enum.sh -i aranumtoolkit/tests/fixtures/services/all-services.xml -o "$tmp_plan" --profile quick --plan-only >/dev/null 2>&1 \
...
rm -rf "$tmp_plan"
python3 aranum.py --help >/dev/null 2>&1 && p "aranum.py: root help succeeds" \
...
out=$(bash aranumtoolkit/network/auto-enum.sh -i aranumtoolkit/network/test.xml -o /tmp/aratool-throttle.$$ --throttle --dry-run 2>&1)
...
out=$(bash aranumtoolkit/network/auto-enum.sh -i aranumtoolkit/network/test.xml -o /tmp/aratool-throttle-x.$$ --throttle --dry-run -P 8 2>&1)
...
# Library helpers
out=$(bash -c '. aranumtoolkit/network/_lib.sh; echo "off=$(throttle_delay)|$(throttle_nmap_args)"; export ENUM_THROTTLE=1; echo "on=$(throttle_delay)|$(throttle_nmap_args)"')
...
check_gate "python3 standalones/activemq/activemq-cve-2023-46604.py --target 127.0.0.1:61616 --cmd id"  "activemq-cve-2023-46604.py (--exploit)"
check_gate "bash standalones/activemq/activemq-jolokia-rce.sh --target 127.0.0.1:8161"                  "activemq-jolokia-rce.sh   (--exploit)"
check_gate "bash standalones/redis/redis-rce-module.sh --target 127.0.0.1:6379"                         "redis-rce-module.sh       (--exploit)"
check_gate "bash standalones/redis/redis-rce-ssh.sh --target 127.0.0.1:6379 --key-inline 'ssh-rsa AAAA x@y'" "redis-rce-ssh.sh      (--write)"
check_gate "bash standalones/smtp/smtp-phish-send.sh --target 127.0.0.1:25 --from a@b --to c@d --subject s --body x" "smtp-phish-send.sh (--send)"
check_gate "python3 standalones/smtp/smtp-smuggling-test.py --target 127.0.0.1:25"                      "smtp-smuggling-test.py    (--send)"
...
out=$(bash aranumtoolkit/network/bulk-enum-linux.sh --targets "$BULK_TGT" -u jay -o "$BULK_OUT" --dry-run 2>&1)
rc=$?
...
out=$(bash aranumtoolkit/network/bulk-enum-linux.sh --targets "$BULK_TGT" -u jay -o "$BULK_OUT2" --throttle -P 4 --dry-run 2>&1)
...
for d in aranumtoolkit/tests/fixtures/bulk-enum/*/; do
    [ -d "$d" ] || continue
...
python3 aranumtoolkit/network/report.py "$BULK_RPT" --label "smoke-bulk" >/dev/null 2>&1 \
...
# -----------------------------------------------------------------
for f in standalones/linux/pwnkit-check.sh standalones/linux/looney-check.sh standalones/linux/overlayfs-check.sh \
         standalones/linux/io-uring-check.sh standalones/linux/namespaces-check.sh standalones/linux/apt-source-check.sh; do
...
EOF
python3 standalones/creds/hash-format.py "$HASH_IN" -o "$HASH_OUT" >/dev/null 2>&1 \
...
out=$(timeout 10 python3 standalones/creds/spray-scheduler.py --users "$SPRAY_USERS" --passwords "$SPRAY_PWS" \
...
cp aranumtoolkit/tests/fixtures/ad-signals/certipy-esc1/certipy_20260520.txt "$ADOUT/ldap/10.0.0.10_389/certipy.txt"
# GPP cpassword (CRITICAL)
cp aranumtoolkit/tests/fixtures/ad-signals/gpp-cpassword/gpp_groups_dump.txt   "$ADOUT/smb/10.0.0.10_445/gpp.txt"
# Unconstrained delegation (HIGH)
cp aranumtoolkit/tests/fixtures/ad-signals/delegation-uncons/delegation_dump.txt "$ADOUT/ldap/10.0.0.10_389/delegation_summary.txt"
# LAPS readable (CRITICAL)
cp aranumtoolkit/tests/fixtures/ad-signals/laps-readable/laps_dump.txt "$ADOUT/ldap/10.0.0.10_389/laps.txt"

python3 aranumtoolkit/network/report.py "$ADOUT" --label smoke-ad >/dev/null 2>&1 \
...
if command -v pwsh >/dev/null 2>&1; then
    for f in standalones/windows/Get-LAPSPassword.ps1 standalones/windows/Get-ADCSMisconfig.ps1 \
             standalones/windows/Get-GPPCPassword.ps1 standalones/windows/Get-DPAPIBlobs.ps1 \
             standalones/windows/Get-NamedPipes.ps1 standalones/windows/Get-PrintNightmare.ps1 \
             standalones/windows/Get-PetitPotamSignals.ps1 standalones/windows/Test-CoercedAuth.ps1; do
...
out=$(python3 aranumtoolkit/network/bulk-enum-windows.py --targets "$WTGT" -u jay -o "$WOUT" --dry-run 2>&1)
rc=$?
...
out=$(python3 aranumtoolkit/network/bulk-enum-windows.py --targets "$WTGT" -u x -o "$WOUT2" --throttle --dry-run 2>&1)
...
out=$(python3 aranumtoolkit/network/bulk-enum-windows.py --targets "$WTGT" -u x -P 6 -o "$WOUT3" --throttle --dry-run 2>&1)
...
out=$(python3 aranumtoolkit/network/bulk-enum-windows.py --targets "$WTGT" -u x --auth basic -o /tmp/wbad --dry-run 2>&1); rc=$?
[ "$rc" -eq 2 ] && p "win: --auth basic over HTTP refused (rc=2)" || f "win: basic-over-HTTP not refused (rc=$rc)"
out=$(python3 aranumtoolkit/network/bulk-enum-windows.py --targets "$WTGT" -u x --use-smb-admin -o /tmp/wbad2 --dry-run 2>&1); rc=$?
[ "$rc" -eq 2 ] && p "win: --use-smb-admin without --pass refused" || f "win: smb-admin sans pass not refused (rc=$rc)"
out=$(python3 aranumtoolkit/network/bulk-enum-windows.py --targets "$WTGT" -u x -P 32 -o /tmp/wbad3 --dry-run 2>&1); rc=$?
...
if bash "$REPO/aranumtoolkit/tests/fp-harness.sh" >/tmp/aratool-fp-harness.log 2>&1; then
