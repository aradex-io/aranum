...
aranumtoolkit/network/enum-*.sh dispatcher fleet honours the contract defined by
aranumtoolkit/network/_lib.sh::parse_common_args.
...
class TestDispatcherContract(unittest.TestCase):
    ...
    def _run(self, svc: str, env_extra: dict | None = None, timeout: int = 15):
        ...
        r = subprocess.run(
            ["bash", str(REPO / "aranumtoolkit" / "network" / f"enum-{svc}.sh"),
    ...
    def test_gated_dispatchers_refuse_without_env(self):
        ...
        for svc in GATED:
            with self.subTest(dispatcher=svc):
                ...
                # hint. err() in aranumtoolkit/network/_lib.sh writes to stdout, not stderr,
...
class TestParseCommonArgsRejection(unittest.TestCase):
    """parse_common_args (aranumtoolkit/network/_lib.sh) must reject unknown args with
    ...
    def _run(self, args):
        r = subprocess.run(
            ["bash", "-c",
             f". {REPO}/aranumtoolkit/network/_lib.sh && parse_common_args {args}"],
