...
# aranumtoolkit/tests/fixtures/ajp-real-nmap.txt without needing a live server.
#
# E4 additions:
...
SCRIPT_DIR="$REPO/aranumtoolkit/tests"
...
for svc in "${DISPATCHERS[@]}"; do
    for i in 0 1 2 3 4 5 6 7; do
        ...
        if [ -n "$extra_env" ]; then
            env "$extra_env" timeout 45 bash "$REPO/aranumtoolkit/network/enum-${svc}.sh" \
                --targets "$tgt" --output "$out" > "$log" 2>&1 || true
        else
            timeout 45 bash "$REPO/aranumtoolkit/network/enum-${svc}.sh" \
                --targets "$tgt" --output "$out" > "$log" 2>&1 || true
        fi
...
_run_gate_test() {
    ...
    env -u "$env_var" timeout 10 bash "$REPO/aranumtoolkit/network/enum-${svc}.sh" \
...
# T4 OT/ICS dispatchers — separate gate function because they live in standalones/ot/
# (not aranumtoolkit/network/) and the gate message they emit is different ("OT_CONFIRMED").
_run_ot_gate_test() {
    local svc="$1"
    ...
    env -u OT_CONFIRMED timeout 10 bash "$REPO/standalones/ot/enum-${svc}.sh" \
        --targets "$tgt" --output "$out" > "$log" 2>&1
    local rc=$?
    ...
    if [ "$rc" -eq 2 ] && [ "$hits" -eq 0 ] && [ "$has_msg" -eq 1 ]; then
        ...
        env_gate_failures=$((env_gate_failures + 1))
        ENV_GATE_CELLS+=("standalones/ot/$svc gate: rc=$rc hits=$hits msg=$has_msg")
    fi
}
...
info "AJP TP: mini integration test against fixture aranumtoolkit/tests/fixtures/ajp-real-nmap.txt"
ajp_fixture="$REPO/aranumtoolkit/tests/fixtures/ajp-real-nmap.txt"
...
PRINT_EXTRA_JETDIRECT_PORT="$jd_tp_port" \
    timeout 30 bash "$REPO/aranumtoolkit/network/enum-print.sh" \
