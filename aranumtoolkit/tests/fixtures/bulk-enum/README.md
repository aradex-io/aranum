# aranumtoolkit/tests/fixtures/bulk-enum/
...
a `linenum.txt` (raw stdout from `standalones/linux/linenum-fast.sh`) plus a stub
`_meta.json` (the layout-detection markers `report.py` looks for).

The four hosts exercise each verdict tier — assertions live in
`aranumtoolkit/tests/test_bulk_enum_report.py` and `aranumtoolkit/tests/smoke.sh` section 13.
