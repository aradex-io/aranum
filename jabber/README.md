# jabber/ — XMPP / Jabber pentesting helpers

Authorized-testing-only. Scope, safety invariants, and gating decisions are recorded in [`docs/ADR-001-19MAY2026-jabber-scope.md`](../docs/ADR-001-19MAY2026-jabber-scope.md). **Read it before using anything in this directory.**
...
|---|---|---|---|
| [`../network/enum-jabber.sh`](../network/enum-jabber.sh) | 7-phase read-only XMPP server enum (banner / cert+SANs / SASL mechs / IBR-advertised / disco / MUC items / BOSH-WS / admin-API) | — | Auto-routed by `auto-enum.sh` from the `xmpp` category |
...
./network/auto-enum.sh -i scan.xml -o ./enum/
...
./jabber/openfire-cve-2023-32315.py --url http://10.0.0.5:9090 exploit \
