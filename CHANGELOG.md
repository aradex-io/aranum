...
#### ADR-006 — bulk-enum overhaul (28JUL2026)

##### Fixed
- `aranumtoolkit/network/bulk-enum-linux.sh`: **`--pass` runs no longer AUTH-FAIL on every host.** `-o BatchMode=yes` was set unconditionally, including on the `sshpass` password path; BatchMode disables the interactive password prompt sshpass answers, so every host failed `Permission denied`. The SSH-option builder now keys `BatchMode`/`PubkeyAuthentication`/`-i` off an explicit auth mode (`KEY`/`PASS`/`KEY_THEN_PASS`) — password mode drops `BatchMode=yes` and adds `NumberOfPasswordPrompts=1`. Verified end-to-end against a real password-auth sshd.

##### Added
- `aranumtoolkit/network/bulk-enum-linux.sh`: three explicit SSH auth modes selected from `--key`/`--pass` (`--key --pass` tries the key first, falls back to the password); per-host status classification (`OK | AUTH_FAIL | UNREACHABLE | TIMEOUT | HOST_TIMEOUT | REMOTE_ERR`) in `_meta.json` (`status`/`fail_reason`) and a new `status` column in `_summary.tsv`; `--preflight`/`--no-preflight` (default on); `--retries N` (retries only transient TIMEOUT/UNREACHABLE, never AUTH_FAIL); `--host-timeout SEC` (default 600, 0=off) wall-clock cap so a slow/tarpit/NFS host can't pin its slot forever; `--jump HOST` shorthand.
- `aranumtoolkit/network/bulk-enum-windows.py`: `--transport {auto,winrm,ssh,smb}` (comma-list/repeatable; `auto` tries winrm→ssh→smb until one authenticates) with a pluggable transport layer; new **ssh** transport ships the PS script over Windows OpenSSH stdin (`--key`, `--ssh-port`), reusing the ADR-006 canonical SSH-option spec so the 1a bug can't recur in Python; formalized **smb** transport (impacket-wmiexec, base64 `-EncodedCommand`, still gated behind `--use-smb-admin`); status classification parity.
- `aranumtoolkit/network/ssh-triage.sh` + `ssh-triage` subcommand: OS-aware SSH sweep dispatcher — classifies each host from `--targets`/`--nmap`/`--nxc` input (nxc-trusted label → SSH banner → authenticated `uname -s`/`ver` probe) and routes Linux hosts to `bulk-enum-linux.sh`, Windows hosts to `bulk-enum-windows.py --transport ssh`; writes `classification.tsv`; unclassified hosts reported, never dispatched. `enum-ssh.sh` gained a shared `classify_ssh_os_from_banner` helper.
- `aranumtoolkit/network/ssh-key-triage.py` + `ssh-key-triage` subcommand: triage a pile of unknown SSH private keys — inventory (type/bits/SHA256+MD5 fingerprint/encrypted?/passphrase-unlock/PEM-vs-OpenSSH/comment via paramiko, `ssh-keygen` fallback) + a non-destructive, publickey-only key×host×user acceptance matrix; emits `key-triage.json`, `key-triage.md`, `authorized-pairs.txt`. Lockout hygiene: `--max-per-user`, `--throttle`, one key per connection, per-engagement known_hosts silo, `--dry-run`. Never attempts password auth or writes to a target.
- `standalones/windows/Get-ThickClientEnum.ps1` + `standalones/linux/thickclient-hunt.sh`: read-only thick-client/workstation enumerators — installed apps, GUI window titles, SSH/PuTTY/WinSCP/FileZilla saved sessions & keys, `.rdp`/cached RDP creds, VPN profiles (OpenVPN/AnyConnect/WireGuard/Forti), browser saved-login DB presence, Electron `app.asar`, DB-client creds (`.pgpass`/`.my.cnf`/DBeaver), keyrings, and secret patterns in app config trees. Inlined (self-contained, per ADR-002 D1) into `linenum-fast.sh` / `Invoke-PrivEscEnum.ps1` so bulk-enum stdin-pipes them; graded by new `THICKCLIENT-*` `report.py` rules.
- Test suites (`aranumtoolkit/tests/`): `test_bulk_enum_linux_auth.py` (per-mode argv + status + host-timeout), `test_bulk_enum_windows_transports.py`, `test_ssh_triage.py`, `test_ssh_key_triage.py`, `test_thickclient_enum.py`. New `make pytest` target runs the pytest-style suites (degrades gracefully when pytest is absent); `make test` now includes it.

##### Enhanced
- `aranumtoolkit/network/report.py`: bulk-enum walk now surfaces the per-host `status` from `_meta.json` — an `AUTH_FAIL`/`UNREACHABLE`/`TIMEOUT`/`HOST_TIMEOUT` host emits a `host NOT fully enumerated` finding instead of an empty `linenum.txt` silently reading as "clean". Added `THICKCLIENT-*` grading rules to both the Linux and Windows bulk rule sets.
- `aranum.py`: registered the `ssh-triage` and `ssh-key-triage` subcommands.

##### Security
- `aranumtoolkit/network/bulk-enum-windows.py`: `--use-smb-admin` help/docstring now states plainly that the smb transport breaks ADR-002 D1's no-on-disk-artifact guarantee (impacket wmiexec stages output via a transient ADMIN$ file).
...
- **TCP service-additions** — `enum-git.sh` (9418 anonymous repo/ref disclosure), `enum-finger.sh` (79 user-enum), `enum-squid.sh` (3128 open-proxy pivot test, `ENUM_PROXY_TEST_URL`), `enum-tftp.sh` (69/udp blind config grab), `enum-x11.sh` (6000-6009 open-display), `enum-afp.sh` (548 server info + shares), `enum-couchbase.sh` (8091 unauth `/pools`), `enum-rethinkdb.sh` (28015 driver/admin). All fully wired: SERVICE_MAP routing, `service-metadata.json`, `report.py` severity rules, and quick-win wiki pages.
- `wiki/` — 11 more quick-win pages for the new dispatchers (ntp, ssdp, mdns, git, finger, squid, tftp, x11, afp, couchbase, rethinkdb); every dispatcher still has a page (`test_wiki.py` coverage assertion).
- `standalones/tomcat/tomcat-war-deploy.sh` (+ README) — new gated Tomcat Manager helper giving the default-creds sweeper's flagship "Tomcat Manager → WAR-deploy RCE" finding a real follow-on. `detect` (read-only default) probes `/manager/text` + validates creds; `--exploit` PUTs an **operator-supplied** `.war` and prints the app URL; `--undeploy` cleans up. Bring-your-own-payload (no webshell bundled), §9-gated, mirrors the openfire/jolokia model.
...
- `aranumtoolkit/docs/SCHEMA.md` — documents the versioned `findings.json` v2 schema (fields, stability contract) and the SARIF export.
- `aranumtoolkit/docs/DATA-SOURCES.md` + `make data-audit` (`aranumtoolkit/tests/data_audit.py`) — provenance index for the embedded offline datasets (default-creds, GTFOBins subset) with an `updated`/`source` on each, and a staleness auditor that warns when a dataset is older than ~9 months.
- `aranumtoolkit/network/report.py --sarif` — writes `findings.sarif` (minimal SARIF 2.1.0: `tool.driver=aranum`, severity→error/warning/note, evidence locations) so findings drop into GitHub code-scanning / DefectDojo / other SARIF consumers.
...
- `standalones/linux/imds-check.sh` — new cloud-metadata (IMDS) reachability auditor: AWS `169.254.169.254` (+ IMDSv1-enabled signal), GCP `metadata.google.internal`, Azure IMDS. Reachability-only — never fetches, prints, or stores credential material (OPSEC §9). curl or bash `/dev/tcp`, dash-safe.
...
### Enhanced
- `aranumtoolkit/network/auto-enum.sh`, `aranum.py`: queue-mode execution-state writeback — a completed service appends its task outcomes to a sibling `queue.state.jsonl` (append-only, so `plan.json`/`queue.jsonl` stay immutable), and `aranum queue --status` now overlays that live state instead of always showing the planner's initial `pending`.
- `aranum.py`: `plan`/`report`/`merge` now print a clear notice when `--session`/`--session-name` is passed (they have no session output layout — use `-o/--output`), instead of silently swallowing the flag.
- `aranumtoolkit/network/auto-enum.sh`: added opt-in cross-service parallelism (`--service-parallel N`, default 1 = serial). Service dispatchers target disjoint host:port sets, so N run concurrently (batched) instead of waiting behind the slowest (usually `enum-http` + nuclei). The OK/FAIL tally is recomputed from each service's `.rc` afterward since backgrounded dispatchers run in subshells; keep it at 1 for throttled/OT runs.
- `aranumtoolkit/network/enum-http.sh`: added marker-gated AD-surface detectors to the product-fingerprint phase — AD CS web enrollment `/certsrv/` (ESC8 NTLM-relay target, cross-linked to `enum-smb.sh`'s relay candidates), Exchange OWA/ECP/Autodiscover/MAPI (ProxyShell/ProxyLogon pre-auth surface), and ADFS `/adfs/ls/` (spray + coercion surface). `report.py` grades them (ESC8 high; Exchange/ADFS medium).
- `standalones/windows/Invoke-PrivEscEnum.ps1`: closed the P0 gap where the all-in-one enumerator (the only script `bulk-enum-windows.py` ships) implemented none of the checks its synopsis advertised. Added CLM-safe registry sections — WSUS-over-HTTP (PyWSUS MITM), UAC token-filter (`LocalAccountTokenFilterPolicy`), LSASS credential posture (WDigest/RunAsPPL/LmCompat/CredGuard), SCCM/MECM NAA path (finally a producer for the orphan CCM `report.py` rule) — a PowerShell `LanguageMode`/downlevel note, and an applicability-gated pure-ADSI AD-depth section (MachineAccountQuota/Certifried, LAPS readability, WebClient coercion-relay). `report.py` `_AD_DEPTH_RULES` grade all of them on the bulk winenum path. Synopsis corrected. Transport still CI-unvalidated per ADR-003.
- `standalones/windows/Get-ADCSMisconfig.ps1`: wrapped the per-template loop body in try/catch (one unreadable template SD no longer aborts the whole enumeration), gated ESC1 on the manager-approval flag (`msPKI-Enrollment-Flag & CT_FLAG_PEND_ALL_REQUESTS`), and `report.py` now grades ESC1-16 (was ESC1-11).
- `aranumtoolkit/network/report.py`: severity classification now uses a combined-alternation prefilter built from the exact same rule patterns, so the per-line hot path short-circuits ~90 individual `re.search` calls into one C-level scan for the (overwhelmingly common) no-match line — a dependency-free speedup at 500+ host scale that provably cannot change any classification (equivalence regression test added).
- `aranumtoolkit/network/enum-redis.sh`: rewrote the shallowest DB dispatcher — reachability classification (UNAUTH vs NOAUTH vs ACL-restricted), TLS/`rediss` retry, `redis_version`+`os`→CVE-2022-0543 (Debian/Ubuntu Lua-sandbox-escape unauth RCE) signal, Sentinel (26379) master-topology probe, `/dev/tcp` fallback (no `nc` dependency), and per-target port (no hardcoded 6379). `report.py` grades the new markers.
- `aranumtoolkit/network/enum-nfs.sh`: emits deterministic `no_root_squash`/`no_all_squash`/`insecure` export markers (the HIGH rule previously relied on the string appearing by luck) and detects NFSv4-only servers (showmount fails with no rpcbind) so an empty result isn't misread as "no shares".
- `standalones/activemq/activemq-cve-2023-46604.py`: added execution-evidence verdicts. The broker's fetch of the staged Spring XML is now surfaced as a `PROGRESSED` verdict (the gadget context loaded), and a new optional `--callback host:port` starts a TCP listener for positive `CONFIRMED` proof (craft `--cmd` to reach back). Exit prints CONFIRMED / PROGRESSED / SENT instead of the old blind "check the broker host".
- `standalones/creds/spray-scheduler.py`: added `--timeout` (per-attempt subprocess timeout, default 300s — the hard-coded 30s killed slow sprays mid-run while still recording the attempt) and `--stop-on REGEX` (captures tool output and halts the whole run on a lockout/success marker, so it's more than a blind pacer). Fixed the misleading docstring example that wired `default-creds-sweep.py` (target-list-driven) with `-u/-p` flags it doesn't accept.
- `aranumtoolkit/network/bulk-enum-windows.py`: implemented the previously-stubbed `--use-smb-admin` SMB fallback (was `rc=126` "not implemented"). It now runs `impacket-wmiexec` delivering the script as an in-memory `powershell -EncodedCommand` (no disk write). WMI's ~8 KB command-line cap means it fits smaller `--script`s only; the default `Invoke-PrivEscEnum.ps1` is too large and returns `rc=126` with actionable guidance rather than silently staging to disk. Added `test_bulk_enum_windows.py` coverage for the no-impacket (127), oversized (126), and small-script-invokes-wmiexec paths. Transport unvalidated in CI per ADR-003.
- `standalones/jabber/jabber-user-enum.py`: implemented the previously-stubbed SCRAM fallback (was "not implemented yet"). When a server offers no SASL PLAIN, it now runs a stdlib SCRAM-SHA-256/1 client-first → server-first → bogus-client-final handshake (crypto verified against the RFC 5802 vector) and maps the result to the same RFC-6120 verdicts, with a documented lower-confidence caveat for anti-enum-hardened servers. Renamed the `NO_PLAIN` verdict to `NO_MECH` (neither PLAIN nor SCRAM offered).
...
- `standalones/linux/writable-files.sh`: added `/etc/ld.so.conf.d/` (dir + `*.conf`) writability (attacker-controlled library search path), broadened the systemd scan beyond `*.service` to `.timer`/`.socket`/`.mount`/`.path`/drop-in `.conf` and to writable `ExecStart=`/`EnvironmentFile=` targets of root units, and flagged `.`/empty/relative PATH elements.
- `standalones/linux/cron-enum.sh`: added writability checks for `cron.{daily,hourly,weekly,monthly}`, `at`-job spools + `atq`, `cron.allow`/`deny`/`at.allow`/`deny`, and wildcard-injection candidates (unquoted `tar */chown -R/…` in root cron).
- `aranumtoolkit/network/report.py`: added default rules so the `[+]`/`[!!]` privesc markers from `container-detect.sh` (docker.sock, privileged CapEff, cgroup-escape primitives, host PID ns, SA token), `capabilities-enum.sh` (`cap_*+ep`), and `group-enum.sh` (docker/lxd/disk/shadow) grade when run individually.
...
### Fixed
- `standalones/smtp/smtp-relay-test.sh`, `smtp-user-enum.sh`: replaced fragile positional reply parsing (`sed -n '3p'` / `head -3|tail -1`) that mistook a multi-line `250-` EHLO extension line for the RCPT/probe outcome. Now keys off the SMTP final-reply convention (a space after the 3-digit code) and takes the last reply before the `221` QUIT, so the verdict is correct regardless of how many EHLO extension lines the server advertises.
- `aranumtoolkit/network/enum-smtp.sh`: the open-relay smoke test tried a single canonical `external→external` form; it now also tries the null-sender, percent-hack, and source-route variants (with the same robust reply detection) and cross-references `standalones/smtp/smtp-relay-test.sh` for the full 19-variant matrix.
- `standalones/redis/redis-rce-ssh.sh`: `--keep` is now honored — it leaves the modified Redis config in place (skipping the on-exit restore) and prints the exact `CONFIG SET` commands to revert manually. Previously the flag was parsed but the config was always restored (TODO).
- `standalones/redis/redis-quickwin.sh`, `_redis_lib.sh`: `--passlist` is now wired through — `try_default_creds` prepends the operator wordlist to its built-in candidates (with fail-loud validation of the file). Previously parsed but unused (TODO).
- `standalones/activemq/activemq-queues.sh`: `--max-msgs` is now applied via Jolokia's `maxCollectionSize` processing option so `browseMessages()` is capped server-side. Previously parsed but unused (TODO).
...
- `standalones/linux/group-enum.sh`, `juicy-files-hunt.sh`: added a fail-loud `bash` guard so invoking them under `dash`/`busybox sh` (they use associative arrays / `mapfile` / process substitution) prints a clear "needs bash" message and exits 2 instead of a confusing parse error.
...
### Changed
- Completed the `aratool`→`aranum` rename for operator-facing strings: dashboard/report titles + theme key (`report.py`, `report-dashboard.py`), `merge-results.py` help/docstring, `CLAUDE.md` title/§1, the MQTT example topic, and test-harness descriptions. Functional identifiers (`/tmp/aratool-*` temp prefixes, the `aratool-probe` RADIUS canary) and historical dated ADR/REVIEW docs are intentionally left unchanged.
...
- `Makefile`: `make lint` now DEGRADES GRACEFULLY when `shellcheck` is absent (e.g. an offline box) — it prints an install notice and falls back to a `bash -n` syntax sweep of every tracked `.sh` (exit 0 when clean) instead of hard-failing with exit 1. This kept `make test` red on any box without shellcheck even though the shell scripts were syntactically fine.
...
### Testing
- `aranumtoolkit/tests/test_report_dashboard.py` — new `TestReportRobustness` adversarial regression suite for `report.py` + `report-dashboard.py`: empty tree, service dir with zero findings, non-UTF8 evidence bytes, a 500 KB single line, malformed `service-metadata.json`, HTML/`<script>` injection in a finding line (asserts it is escaped), odd service/host directory names (asserts no generated page escapes `--output`), `--redact` completeness (asserts zero raw dotted-quad IPs survive), and OPSEC §9 symlink containment (a symlink pointing outside the scan tree must not be ingested). Every case must exit 0 with well-formed HTML + valid `data.json`. Locks in the offline battle-test hardening so the generators can never again regress into crashing on bad engagement data.
- `aranumtoolkit/tests/test_bulk_enum_windows.py` — the four WinRM-transport tests (`TestWinRMEndpoint` + `test_mocked_winrm_writes_per_host_output`) now `skipUnless` the optional `winrm` (pywinrm) module is importable. They `mock.patch("winrm.Session", …)`, which forces an import of `winrm` to resolve the patch target, so on an offline / stdlib-only box they used to **ERROR** (`ModuleNotFoundError: No module named 'winrm'`) rather than skip — making a clean box read as broken. Assertions are unchanged when pywinrm is present.
