...
- ActiveMQ `admin:admin` → Jolokia MBean RCE (see `standalones/activemq/` toolkit)
...
python3 ../../aranumtoolkit/network/nmap-parse.py scan.xml --all-ports | awk '{print $1}' > targets.txt
...
- **SNMP communities** — see `aranumtoolkit/network/enum-snmp.sh` with `onesixtyone`.
